...
(async()=>{const browser=await chromium.launch({executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe',headless:true});try{
 ...
 for(const symbol of ['Hg','Tl','Pb','Bi','Po','At','Rn']){
 ...
 const routes=[['Au','Hg'],['Hg','Rn'],['Rn','H'],['Pb','Cu'],['Bi','Po'],['At','Rn'],['Rn','Au']];
 ...
 const identities=await p.evaluate(()=>{const C=Copper;let n=0;for(let z=80;z<=86;z++)for(const field of ['name','symbol']){const e=structuredClone(C.elements[z]);e[field]='Wrong';if(C.validatePeriod6CompletionData({[z]:e}).ok)throw Error('Identity accepted');n++;}return n;});assert.equal(identities,14);
 await p.goto(url+'#explore=Rn');await p.waitForFunction(()=>Copper.app.atom.element.symbol==='Rn');await p.click('#fullscreen');
 ...
 await p.locator('#stage').screenshot({path:'qa/phase6k-Rn-fullscreen.png'});await p.click('#exit-fullscreen');assert.equal(await p.evaluate(()=>Copper.app.state.presentation),false);
 assert.deepEqual(errors,[]);assert.deepEqual(remote,[]);const out={result:'PASS',doubleClicks,mouseGestures,identityNegativeCases:identities,fullscreen,errors,remote};fs.writeFileSync('qa/phase6k-state-results.json',JSON.stringify(out,null,2));console.log(out);
