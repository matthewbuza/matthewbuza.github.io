...
(async()=>{const browser=await chromium.launch({executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe',headless:true});try{
 ...
 for(const symbol of ['Hf','Ta','W','Re','Os','Ir','Pt','Au']){
 ...
 const routes=[['H','Au'],['Cu','W'],['Lu','Hf'],['Pt','Au'],['Au','H']];
 ...
 const identities=await p.evaluate(()=>{const C=Copper;let n=0;for(let z=72;z<=79;z++)for(const field of ['name','symbol']){const e=structuredClone(C.elements[z]);e[field]='Wrong';if(C.validateHeavyDData({[z]:e}).ok)throw Error('Identity accepted');n++;}return n;});assert.equal(identities,16);
 await p.goto(url+'#explore=Au');await p.waitForFunction(()=>Copper.app.atom.element.symbol==='Au');await p.click('#fullscreen');
 ...
 await p.locator('#stage').screenshot({path:'qa/phase6j-Au-fullscreen.png'});await p.click('#exit-fullscreen');assert.equal(await p.evaluate(()=>Copper.app.state.presentation),false);
 assert.deepEqual(errors,[]);assert.deepEqual(remote,[]);const out={result:'PASS',doubleClicks,mouseGestures,identityNegativeCases:identities,fullscreen,errors,remote};fs.writeFileSync('qa/phase6j-state-results.json',JSON.stringify(out,null,2));console.log(out);
