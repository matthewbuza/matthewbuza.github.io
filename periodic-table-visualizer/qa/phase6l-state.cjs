...
(async()=>{const browser=await chromium.launch({executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe',headless:true});try{
 ...
 const routes=[['Rn','Ac'],['Ac','Th'],['U','Pu'],['Pu','Am'],['Am','H'],['Cu','U'],['Am','Rn']];
 ...
 const identities=await p.evaluate(()=>{const C=Copper;let n=0;for(let z=89;z<=95;z++)for(const field of ['name','symbol']){const e=structuredClone(C.elements[z]);e[field]='Wrong';if(C.validateActinideData({[z]:e}).ok)throw Error('Identity accepted');n++;}return n;});assert.equal(identities,14);
 await p.goto(url+'#explore=Am');await p.waitForFunction(()=>Copper.app.atom.element.symbol==='Am');await p.click('#fullscreen');
 ...
 await p.locator('#stage').screenshot({path:'qa/phase6l-Am-fullscreen.png'});await p.click('#exit-fullscreen');assert.equal(await p.evaluate(()=>Copper.app.state.presentation),false);
 assert.deepEqual(errors,[]);assert.deepEqual(remote,[]);const out={result:'PASS',doubleClicks,mouseGestures,identityNegativeCases:identities,fullscreen,errors,remote};fs.writeFileSync('qa/phase6l-state-results.json',JSON.stringify(out,null,2));console.log(out);
